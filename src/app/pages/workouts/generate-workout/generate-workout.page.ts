...
import {
  ...
  intensity,
  wLocation,
} from 'src/app/data/workout-data/CreateWorkout';
...
export class GenerateWorkoutPage implements OnInit {
  ...
  async getWorkoutDetails(wid, uid) {
    ...
    this.workoutService.getWorkout(wid, uid).subscribe((results) => {
      ...
      this.workoutDesc.intensity = this.listIntensity.find(
        (x) => x.value === this.workoutDesc.intensity
      ).text;
      this.workoutDesc.duration = this.listDuration.find(
        (x) => x.value === this.workoutDesc.duration
      ).info;
      this.workoutDesc.equipment = this.listEquipment.find(
        (x) => x.value === this.workoutDesc.equipment
      ).text;
      this.workoutDesc.location = this.listLocation.find(
        (x) => x.value === this.workoutDesc.location
      ).text;

      this.exerciseList = this.workoutDesc.workoutRoutine;

      if (this.workoutDesc.duration === '15 Mins') {
        this.warmUpTime = '2.5 mins';
        this.coolDownTime = '2.5 mins';
        this.workoutTime = '10 mins';
      }
      if (this.workoutDesc.duration === '30 Mins') {
        this.warmUpTime = '5 mins';
        this.coolDownTime = '5 mins';
        this.workoutTime = '20 mins';
      }
      if (this.workoutDesc.duration === '60 Mins') {
        this.warmUpTime = '5 mins';
        this.coolDownTime = '5 mins';
        this.workoutTime = '50 mins';
      }
      if (this.workoutDesc.duration === '90 Mins >') {
        this.warmUpTime = '10 mins';
        this.coolDownTime = '10 mins';
        this.workoutTime = '70 mins';
      }
    });
   ...
   */
  async goGymMap() {
    await this.router.navigateByUrl('tabs/workouts/display-gyms', {
      replaceUrl: true,
    });
  }
   ...
   */
  async startWorkout() {
    await this.router.navigate(['/start-workout'], {
